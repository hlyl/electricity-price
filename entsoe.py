...
class EntsoeClient:
    def __init__(self, token):
        self.base_url = "https://transparency.entsoe.eu"
        self.token = token

    def day_ahead_prices(self, domain, start_time, end_time, vat=0.0):
        """
        ...
        period_start = (
            ...
            .strftime("%Y%m%d%H%M")
        )  # API expects UTC timestamps with full hours
        period_end = (
        ...
        endpoint = "/api?securityToken={0}&documentType={1}&in_Domain={2}&out_Domain={2}&periodStart={3}&periodEnd={4}".format(
            self.token, document_type, domain, period_start, period_end
        )
        ...
        for time_series in xml_root.findall("d:TimeSeries", namespaces=xml_ns):
            if (
                time_series.findtext("d:currency_Unit.name", namespaces=xml_ns) != "EUR"
                or time_series.findtext("d:price_Measure_Unit.name", namespaces=xml_ns)
                != "MWH"
            ):
                raise ValueError("Unexpected currency or measure unit")
            start = datetime.datetime.strptime(
                time_series.findtext(
                    "./d:Period/d:timeInterval/d:start", namespaces=xml_ns
                ),
                "%Y-%m-%dT%H:%MZ",
            ).replace(tzinfo=datetime.timezone.utc)
            end = datetime.datetime.strptime(
                time_series.findtext(
                    "./d:Period/d:timeInterval/d:end", namespaces=xml_ns
                ),
                "%Y-%m-%dT%H:%MZ",
            ).replace(tzinfo=datetime.timezone.utc)
            ...
            for point in time_series.findall("./d:Period/d:Point", namespaces=xml_ns):
                ...
                if pos > skip_begin and pos <= 24 - skip_end:
                    prices.append(
                        {
                            "time": start + (pos - 1) * datetime.timedelta(hours=1),
                            "price": (1 + vat) * price / 1000,
                        }
                    )  # Add VAT, convert to EUR/kWh
